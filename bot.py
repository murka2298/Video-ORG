...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "2) Videoning boshqa formatdagi (mov) nusxasini\n"
...
def make_ios_compatible(src_path: str, out_dir: str) -> str:
    ...
    (H.264 + AAC, faststart metadata, doimiy kadr chastotasi)."""
    dst_path = os.path.join(out_dir, "final.mp4")
    cmd = [
        FFMPEG_PATH, "-y", "-fflags", "+genpts", "-i", src_path,
...
def convert_video_format(src_path: str, out_dir: str, ext: str = "mov") -> str:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            converted_path = convert_video_format(final_video_path, tmp_dir, ext="mov")
        except Exception:
        ...
        # Konvertatsiya qilingan video (mov)
        if converted_path and os.path.exists(converted_path):
            if os.path.getsize(converted_path) <= MAX_TELEGRAM_SIZE:
                with open(converted_path, "rb") as f:
                    await update.message.reply_video(
                        f, caption="Boshqa formatdagi video (mov)", supports_streaming=True
                    )
            else:
